  ...
  describe("findWalletHistoryService", () => {
      it("should find wallet history entries with the given payload", async () => {
          const mockFind = jest.fn().mockReturnValue({
              sort: jest.fn().mockResolvedValueOnce([mockWalletHistory]), // Return sorted entries
          });

          (WalletHistory.find as jest.Mock).mockImplementation(mockFind); // Mock the find method
