...
const allowStableTokenAddressfForSaving =asyncHandler(async(req:Request,res:Response)=>{
    ...
    try{
        ...
        const tx = await addSupportedTokenAddress(stableTokenAddress,userPrivateKey);
        if (!tx){
...
const unlistStableTokenAddressfForSaving =asyncHandler(async(req:Request,res:Response)=>{
    const {stableTokenAddress}=req.body
    //@ts-ignore
    const userId = req.user.userId;
    try{
...
const changeSavingContractOwnership =asyncHandler(async(req:Request,res:Response)=>{
...
const setFeeCollectionAddress =asyncHandler(async(req:Request,res:Response)=>{
    ...
    try{
        ...
        const userPrivateKey = decrypt(wallet.encryptedKey);
        const tx = await setAddressForFeeCollection(feeCollectionAddress,userPrivateKey);
        if (!tx){
...
export {setFeeCollectionAddress,changeSavingContractOwnership,unlistStableTokenAddressfForSaving,allowStableTokenAddressfForSaving}