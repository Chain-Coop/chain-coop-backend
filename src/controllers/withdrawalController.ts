	...
	try {
		...
		if (!amount || !accountNumber || !bankCode || !pin) {
			return res
				...
				.json({ status: StatusCodes.BAD_REQUEST, error: "Amount, account number, bank code, and pin are required" });
		}

		// Validate the wallet
		const userWallet = await findWalletService({ user: userId });
		if (!userWallet) {
			return res.status(StatusCodes.NOT_FOUND).json({ status: StatusCodes.NOT_FOUND, error: "Wallet not found" });
		}

		if (!userWallet.isPinCreated) {
			return res.status(StatusCodes.BAD_REQUEST).json({ status: StatusCodes.BAD_REQUEST, error: "Pin not set for the wallet" });
		}
		...
		if (!accountDetails) {
			return res.status(StatusCodes.BAD_REQUEST).json({ status: StatusCodes.BAD_REQUEST, error: "Invalid bank account details" });
		}
		...
		return res
			...
			.json({
				status: error instanceof BadRequestError
...
export const updateWithdrawalStatusController = async (req: Request, res: Response) => {
	try {
		...
		if (!validStatuses.includes(status)) {
			return res
				.status(StatusCodes.BAD_REQUEST)
				.json({ status: StatusCodes.BAD_REQUEST, error: "Invalid status. Status must be one of: pending, completed, failed" });
		}
		...
		if (!withdrawal) {
			return res
				...
				.json({ status: StatusCodes.NOT_FOUND, error: "Withdrawal request not found" });
		}
