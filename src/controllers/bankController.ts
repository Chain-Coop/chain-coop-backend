...
// Define interfaces for the expected Paystack response structure
interface Bank {
	name: string;
	code: string;
}
...
export const getAllBanks = async (req: Request, res: Response) => {
	try {
		...
		const banks = response.data.data; // Now typed as Bank[]
...
export const verifyBankAccount = async (req: Request, res: Response) => {
	...
	try {
		const response = await axios.get<{ data: BankAccountDetails }>(
