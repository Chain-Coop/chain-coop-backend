 ...
 *
 *       Content handling for `html` and `text`:
 *       - At least one of `html` or `text` is required.
 *       - If only `html` is provided: the HTML is sent as-is; a plain-text alternative is auto-generated by stripping HTML tags.
 *       - If only `text` is provided: the text is wrapped into a minimal HTML `<pre style="white-space:pre-wrap;font-family:Arial,sans-serif;color:#222;">…</pre>` for HTML clients, and the same text is sent as the plain-text alternative.
 *       - If both are provided: `html` is used as the main body and `text` as the plain-text alternative.
 *       - No design template or branding wrapper is applied for raw sends.
 *     tags: [Email]
 *     requestBody:
 *       required: true
 *       content:
